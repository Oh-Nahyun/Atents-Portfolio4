...
public class RoadTile : Tile
{
    ...
    /// <summary>
    /// 타일맵의 RefreshTile 함수가 호출될 때 호출, 타일이 어떤 스프라이트를 그릴지 결정하는 함수
    /// </summary>
    ...
    public override void GetTileData(Vector3Int position, ITilemap tilemap, ref TileData tileData)
    {
        ...
        // 북쪽과 서쪽에 길이 있으면 mask의 최종값은? 1001
        ...
        if (index > -1 && index < sprites.Length) // 인덱스가 제대로 골라졌는지 확인
        {
            ...
            matrix.SetTRS(Vector3.zero, GetRotation(mask), Vector3.one); // 타일 회전시키기
    ...
    /// <returns>true면 같은 종류의 타, false면 다른 종류의 타일</returns>
    ...
    int GetIndex(AdjTilePosition mask)
    {
        ...
        switch (mask)
        {
            ...
            case AdjTilePosition.All & ~AdjTilePosition.West:     // 0000 1111 & ~(0000 1000) = 0000 0111 // 서쪽만 빼고
                                                                  // all : 0000 1111, ~north : ~(0000 0001) = 1111 1110
    ...
    {
        string path = EditorUtility.SaveFilePanelInProject( // 파일 저장창을 열고 입력 결과를 돌려주는 함수
                                    "Save Road Tile", // 제목
