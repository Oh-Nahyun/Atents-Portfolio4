    ...
    public static List<Vector2Int> PathFind(GridMap map, Vector2Int start, Vector2Int end)
    {
        ...
        if (map.IsValidPosition(start) && map.IsValidPosition(end) && !map.IsWall(start) && !map.IsWall(end))
        {
            ...
            while (open.Count > 0) // open 리스트에 노드가 있으면 계속 반복
            {
                ...
                open.RemoveAt(0);   // open 리스트에서 제일 앞에 있는 노드를 제거

                if (current != end)
                {
                    // 목적지가 아니다.
                    close.Add(current); // close 리스트에 current를 추가해서 탐색이 완료되었음을 표시
            ...
            if (current == end)
            {
                ...
                path.Reverse(); // 도착점 -> 시작점으로 되어있는 경로를 뒤집기
            }
        }
