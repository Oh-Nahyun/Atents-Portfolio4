...
public class TileGridMap : GridMap
{
    ...
    public TileGridMap(Tilemap background, Tilemap obstacle)
    {
        ...
        width = background.size.x;                                  // background 타일맵의 크기를 가로, 세로 길이로 지정
        ...
        for (int y = min.y; y < max.y; y++)
        {
            ...
            {
                if (GridToIndex(x, y, out int? index))              // 인덱스 값 구하기
                {
                    ...
                    {
                        moveable.Add(new Vector2Int(x, y));         // 이동 가능한 지역 저장
                    }
        ...
        moveablePositions = moveable.ToArray(); // 임시 리스트를 배열로 저장
    }
    ...
    /// <returns>true면 맵 , false면 맵 밖</returns>
    ...
    /// </summary>
    /// <returns>이동 가능한 위치</returns>
    public Vector2Int GetRandomMoveablePosition()
    {
        // 슬라임이 목적지에 도착하면 새로운 목적지를 랜덤으로 설정
        /* // 내가 작성한 코드
        int randInt = UnityEngine.Random.Range(-9, 9);
        Vector2Int randDestination = new Vector2Int(randInt, randInt);
    ...
    /// </summary>
    /// <param name="worldPosition">확인할 위치 (월드 좌표)</param>
