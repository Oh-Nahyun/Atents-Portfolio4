...
public class Slime : RecycleObject
{
    ...
    public void Die()
    {
        isMoveActicate = false;             // 이동 비활성화
        onDie?.Invoke();                    // 죽었음을 알리기
        onDie = null;                       // 연결된 함수 모두 제거
        StartCoroutine(StartDissolve());    // 디졸브만 실행 (디졸브 코루틴 안에서 비활성화까지 처리)
    }
    ...
    public void ShowOutline(bool isShow = true)
    {
        ...
        if (isShow)
        {
            mainMaterial.SetFloat(OutlineThicknessID, VisibleOutlineThickness); // 보이는 것은 두께를 설정하는 것으로 보이게 만듦
        }
        else
        {
            mainMaterial.SetFloat(OutlineThicknessID, 0); // 안보이는 것은 두께를 0으로 만들어서 안보이게 만듦
        }
    }
    ...
    IEnumerator StartPhase()
    {
        ...
        while (timeElapsed < phaseDuration) // 시간 진행에 따라 처리
        {
    ...
    IEnumerator StartDissolve()
    {
        ...
        while (timeElapsed < dissolveDuration)
        {
            ...
            //mainMaterial.SetFloat(DissolveFadeID, 1 - (timeElapsed / phaseDuration));
    ...
    void MoveUpdate()
    {
        if (isMoveActicate)
        {
            if (path.Count > 0 && pathWaitTime < MaxPathWaitTime) // 경로가 남아있고 오래 기다리지 않았을 때의 처리
            {
                // path의 첫번째 위치로 계속 이동
                /* ///// 내가 시도한 코드
                for (int i = 0; i < path.Count; i++)
                {
                    transform.position = map.GridToWorld(path[i]);
                ...
                */
                ...
                {
                    // 실제로 이동하는 처리
                    Vector3 destPosition = map.GridToWorld(destGrid);       // 목적지 월드 좌표 구하기
                    Vector3 direction = destPosition - transform.position;  // 방향 계산

                    if (direction.sqrMagnitude < 0.001f)                    // 방향 벡터의 길이를 확인해서 도착했는지 확인
                    {
                        // 첫번째 위치에 도착
