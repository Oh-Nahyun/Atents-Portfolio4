...
public class WorldManager : MonoBehaviour
{
    ...
    /// <summary>
    /// 모든 씬이 언로드 되었음을 확인하기 위한 프로퍼티 (모든 씬이 Unload 상태이면 true, 아니면 false)
    /// </summary>
    ...
    public void Initialize()
    {
        ...
        if (player != null)
        {
            ...
            Vector2Int grid = WorldToGrid(player.transform.position); // 플레이어가 있는 맵의 그리드 값 가져오기
    ...
    void RequestAsyncSceneUnload(int x, int y)
    {
        ...
        {
            unloadWork.Add(index); // 로딩 완료된 씬만 unload 리스트에 추가
        }
    }
    ...
    private void Update()
    {
        ...
        {
            loadWork.RemoveAll( (x) => x == index); // loadWork 리스트에서 index와 같은 아이템은 모두 제거
        }
        ...
        {
            unloadWork.RemoveAll((x) => x == index); // unloadWork 리스트에서 index와 같은 아이템은 모두 제거
        }
    ...
    /// </summary>
    /// <param name="mapX">지정된 맵의 x위치</param>
    /// <param name="mapY">지정된 맵의 y위치</param>
    void RefreshScenes(int mapX, int mapY)
    {
        ...
        int startX = Mathf.Max(0, mapX - 1); // 최소점은 (mapX, mapY)보다 1 작거나 (0, 0)
        int startY = Mathf.Max(0, mapY - 1);
        int endX = Mathf.Min(WidthCount, mapX + 2); // 최대점은 (mapX, mapY)보다 1 크거나 (WidthCount, HeightCount)
